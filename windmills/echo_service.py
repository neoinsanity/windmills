...
class EchoService(Cornerstone):
    ...
    Beginning run() with state: <....EchoService object at ...> and configuration:  Namespace(heartbeat=3, log_level='error', message=None, monitor_stream=False, name='EchoService', no_block_send=False, reply_sock_url='tcp://localhost:8889', verbose=True)
    >>> time.sleep(1)
    >>> assert t.is_alive()
    >>> foo.kill()
    >>> t.join(1)
